    ...
    useEffect(() => {
        ...
        const animate = (currentTime: number) => {
            ...
            if (!canvas || !ctx || !particlesRef.current.length) {
                // If particles empty but state is vaporize, complete it instantly
                if (particlesRef.current.length === 0 && animationState === "vaporizing") {
...
const createParticles = (
    ...
    // Process standard single-line and multiline rendering seamlessly
    ...
    const lineHeight = fontSizeMatch ? parseInt(fontSizeMatch[1], 10) * 1.5 : 50 * 1.5;
    const startY = textY - (lines.length - 1) * lineHeight / 2;

    // Render text to canvas handling multiline
